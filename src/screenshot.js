	...
	try {
		...
		if (device) {
			...
			if (devices[device]) {
				...
				console.warn(chalk.yellow(`  Warning: Unknown device "${device}", using default viewport`));
			}
		}
		...
		if (selector) {
			...
			// Page screenshot
			if (verbose) console.log(chalk.dim(`  Capturing ${fullPage ? 'full page' : 'viewport'}...`));
		...
		if (!quiet) {
			...
			if (fullPage) {
				...
				console.log(chalk.dim(`  Dimensions: ${dimensions.width}x${dimensions.height}px`));
			}
			console.log('');
		}
