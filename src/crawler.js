...
export class Crawler extends EventEmitter {
  ...
  async _crawl(url, remainingDepth) {
    ...
    if (this.options.maxTime && Date.now() - this.startTime >= this.options.maxTime) {
      return;
    }
  ...
  async _capturePage(url, remainingDepth) {
    ...
    try {
      ...
      const rewrittenHtml = rewriteLinks(
        result.html,
        url,
        urlMap,
        { 
  ...
  async _rewriteCssFiles(resources, urlMap) {
    ...
    const { getRelativePath, joinPath, sanitizePath } = await import('./utils/path.js');
    ...
    for (const [resourceUrl, resource] of resources) {
      ...
      try {
        ...
        const cssLocalPath = joinPath(this.options.output, sanitizePath(cssRelativePath));
        ...
        this.logger.debug(`Failed to rewrite CSS ${resourceUrl}:`, error.message);
      }
    }
  }
}
