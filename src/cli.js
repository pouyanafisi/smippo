...
import { showHelp, showBanner, runInteractiveCapture, shouldRunInteractive } from './interactive.js';
...
export function run() {
	...
	program
		.name('smippo')
		.description('Modern website copier powered by Playwright - capture JS-rendered pages for offline viewing')
		.version(version);
	...
	program
		...
		// Scope options
		.option('-s, --scope <type>', 'Link scope: subdomain|domain|tld|all', 'domain')
		...
		// Browser options
		.option('--wait <strategy>', 'Wait strategy: networkidle|load|domcontentloaded', 'networkidle')
		...
		// Output options
		.option('--structure <type>', 'Output structure: original|flat|domain', 'original')
	...
	program
		...
		.option('-p, --port <port>', 'Port to serve on (auto-finds available)', '8080')
	...
	program
		...
		.option('-O, --out <file>', 'Output file path (auto-generated if not specified)')
		...
		.action(async (url, options) => {
			try {
				const { captureScreenshot, parseViewport } = await import('./screenshot.js');
...
async function capture(url, options) {
	...
	crawler.on('page:complete', ({ url, size }) => {
		if (options.verbose) {
			spinner.succeed(`Captured: ${truncateUrl(url, 50)} (${formatSize(size)})`);
			spinner.start();
		}
	});
...
async function continueCapture(options) {
	if (!manifestExists(options.output)) {
		throw new Error('No capture found in the specified directory. Start a new capture first.');
	}
