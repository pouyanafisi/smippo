...
export function createProgressDisplay(options = {}) {
	...
	const multibar = new cliProgress.MultiBar(
		{
			...
			format: (options, params, payload) => {
				...
				const bar = options.barCompleteChar.repeat(Math.round(params.progress * 30));
				const empty = options.barIncompleteChar.repeat(30 - Math.round(params.progress * 30));
	...
	return {
		...
		start(estimatedPages = 1, estimatedAssets = 10) {
			...
			currentBar = multibar.create(100, 0, { type: 'current', name: 'Initializing...' });
		},
